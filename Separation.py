...
def separation(week, gameid, playid, team = 'both'):
    ...
    dist_to_ball = {}
    football = week[(week['playId']== playid) & (week['gameId'] == gameid) & (week['displayName'] == 'Football')  & ((week['event'] == 'pass_outcome_caught')|(week['event'] == 'pass_outcome_incomplete')| (week['event'] == 'pass_outcome_interception') | (week['event'] == 'qb_sack') | (week['event'] == 'qb_strip_sack') | (week['event'] == 'pass_outcome_touchdown'))]
    if football.empty:
        print(week, gameid, playid)
    else:
        ball_pos = football.loc[:, ['x']]['x'].item(), football.loc[:, ['y']]['y'].item()
    for x in players:
        for index, row in week[(week['playId']== playid) & (week['gameId'] == gameid) & (week['displayName'] != 'Football') & (week['position'] == x) & ((week['event'] == 'pass_outcome_caught')|(week['event'] == 'pass_outcome_incomplete')| (week['event'] == 'pass_outcome_interception') | (week['event'] == 'qb_sack') | (week['event'] == 'qb_strip_sack') | (week['event'] == 'pass_outcome_touchdown'))].iterrows():
    ...
    for key, value in dist_to_ball.items():
        if value < lowest_dist:
            lowest_dist = value
            lowest_name = key
    return lowest_name, lowest_dist#, week['event']
#print(separation(week1, 2018090600, 75, 'defense'))
...
for a in [eval('week' + str(x)) for x in range(1, 17)]:
    for b in a['gameId'].unique():
        for c in a[a['gameId'] == b]['playId'].unique():
            print(nearest_defender(a, b, c), c)

nearest_defender(a, b, c)
